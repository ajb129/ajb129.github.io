...
<xsl:if test="$mode=''">
	<html>
		<head>
			...
			<title><xsl:value-of select='fn:header/fn:corpus/@description'/></title>
			<style>
...
<xsl:if test="$mode='sentence'">
    ...
    var finalStr = ["", ""];
    //]]>
    var nameId = "<xsl:value-of select='//fn:sentence/fn:annotationSet[@ID=$sentId]/../@ID' />";
    ...
    pretext = "" + nameId + " [<a href='javascript:removeSent(" + sentId + ")'>X</a>] ";
